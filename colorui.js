(function(){
  fetch('colors.json?cb='+Date.now()).then(function(r){return r.json();}).then(function(data){
    var C=data.colors||{}, days=data.days||[];
    ...
    function show(name,x,y){
      var cd=C[name]; if(!cd){hide();return;}
      ...
      var h='<span class="cpclose">✕</span><h4>'+name+'</h4><div class="cpsub">컬러별 판매 · '+range+' · 총 '+tot+'장</div>';
    ...
    document.addEventListener('click',function(e){
      ...
      if(tr){var td=tr.querySelector('td');if(td){var nm=clean(td.textContent);if(C[nm]){show(nm,e.clientX,e.clientY);return;}}}
      hide();
    });
